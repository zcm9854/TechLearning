...
class VideoListVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(ViedeoListCell.self, forCellReuseIdentifier: "cell")
    ...
    func getList() {
        ...
        Alamofire.request("https://api.avgle.com/v1/videos?page=\(self.page)&c=\(self.category)", method: .get).responseJSON {
            [weak self] response in
            guard let json = response.result.value else { return }
            guard let this = self else { return }
            MBProgressHUD.hide(for: this.view, animated: true)
            let result = JSON(json)
            this.data = result["videos"].arrayValue.map({ js in
